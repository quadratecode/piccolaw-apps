...
# Custom config
config(
    title="Web App Arbeitsrecht | Web App Employment Law",
...
def check_form_employment(data):
    try:
        ...
        return (
            "employment_sdt",
            lang("ERROR: Invalid date", "ERROR: Ungültiges Datum"),
        )
...
def check_case_comb(data):
    if (
        ...
        and (data["trial_relevance"] == False)
        and (data["termination_occurence"] == False)
    ):
        output.put_error(
            lang("ERROR: No parameter selected", "ERROR: Kein Parameter ausgewählt"),
            closable=True,
...
def check_form_incapacity(data):
    ...
    if data_lst_sorted != data_lst:
        output.put_error(
            lang(
                "ERROR: Dates not in chronological order",
...
def check_trial(data):
    if len(data["workdays_input"]) < 1:
        output.put_error(
            lang("ERROR: No days selected", "ERROR: Keine Arbeitstage ausgewählt"),
            closable=True,
            scope="scope_input_instructions",
        )
        return ("", "")
...
def check_form_termination(data):
    ...
    if employment_sdt > arrow.get(data["termination_dt"], "DD.MM.YYYY"):
        output.put_error(
            lang(
                "ERROR: Termination date cannot be older than the employment start date. Start date: "
                + str(employment_sdt.format("DD.MM.YYYY")),
                "ERROR: Das Kündigungsdatum kann nicht vor dem Startdatum liegen. Startdatum: "
                + str(employment_sdt.format("DD.MM.YYYY")),
            ),
            closable=True,
            scope="scope_input_instructions",
...
def holiday_checker(day, workplace):
    ...
    if (
        ...
        # Berchtoldstag
        or (
            day == arrow.Arrow(day.year, 1, 2)
            and workplace in cantons_holidays["berchtoldstag"]
        )
        # Heilige Drei Könige
        or (
            day == arrow.Arrow(day.year, 1, 6)
            and workplace in cantons_holidays["three_kings"]
        )
        # Jahrestag der Ausrufung der Republik Neuenburg
        or (
            day == arrow.Arrow(day.year, 3, 1)
            and workplace in cantons_holidays["republic_neuenburg"]
        )
        # Josefstag
        or (
            day
            == (
                arrow.Arrow(day.year, 3, 19)
                and workplace in cantons_holidays["josefstag"]
            )
        )
        # Karfreitag
        or (
            day == easter_dt.shift(days=-2)
            and workplace in cantons_holidays["karfreitag"]
        )
        # Ostermontag
        or (
            day == easter_dt.shift(days=+1)
        ...
        or (
            ...
            and workplace in cantons_holidays["labour_day"]
        )
        # Auffahrt
        or (
            day == easter_dt.shift(days=+39)
            and workplace in cantons_holidays["ascension"]
        )
        # Pfingstmontag
        or (
...
def create_input_fields(num_periods):
    ...
    for i in range(1, num_periods + 1):
        ...
        input_fields.append(
            input.input(
                lang(f"Period {i} - Start", f"Periode {i} - Beginn"),
                name=f"illacc_sdt_{i}",
                type=input.TEXT,
                required=required,
        ...
        input_fields.append(
            input.input(
                lang(f"Period {i} - End", f"Periode {i} - Ende"),
                name=f"illacc_edt_{i}",
                type=input.TEXT,
                required=required,
                pattern=DATE_PATTERN,
                maxlength="10",
...
def emplaw_app():
    ...
    # Define date pattern
    # Make global so that it can be used in inpuft functions
    global DATE_PATTERN
    DATE_PATTERN = "[0-9]{2}\.[0-9]{2}\.(19|20)\d{2}$"

    # --- SESSION CONTROL --- #
    session.set_env(input_panel_fixed=False, output_animation=False)
    ...
    with output.use_scope("scope_input_instructions"):
        output.put_markdown(
            lang(
                instructions["technical_info"]["EN"],
                instructions["technical_info"]["DE"],
            )
        ...
        output.put_collapse(
            ...
            [
                output.put_html(
                    lang(
                        instructions["legal_framework"]["EN"],
                        instructions["legal_framework"]["DE"],
                    )
                )
            ],
        ).style("margin-top: 20px")
        ...
        output.put_markdown(
            lang(
                instructions["terms_and_conditions"]["EN"],
                instructions["terms_and_conditions"]["DE"],
            )
        )
    ...
    input.actions(
        ...
        [
            {
                "label": lang("Agree and continue", "Zustimmen und fortfahren"),
                "value": "continue",
            },
        ],
    )
    ...
    with output.use_scope("scope_progress"):
        output.put_processbar("bar", init=0, scope="scope_progress", position=0).style(
            "margin-top: 30px"
        )
    ...
    with output.use_scope("scope_input_instructions", clear=True):
        output.put_markdown(
            lang(
                instructions["general_employment_data"]["EN"],
                instructions["general_employment_data"]["DE"],
            )
        )
    ...
    employment_data = input.input_group(
        ...
        [
            input.input(
                lang("First day of work", "Tag des Stellenatritts"),
                name="employment_sdt",
                type=input.TEXT,
                required=True,
                pattern=DATE_PATTERN,
            ...
            input.select(
                ...
                cantons_holidays["all_cantons_code"],
                name="workplace",
                type=input.TEXT,
                required=True,
            ),
    ...
    with output.use_scope("scope_input_instructions", clear=True):
        output.put_markdown(
            lang(
                instructions["case_combinations"]["EN"],
                instructions["case_combinations"]["DE"],
            )
        )
    ...
    case = input.input_group(
        ...
        [
            input.select(
                lang("Type of incapacity", "Art der Arbeitsunfähigkeit"),
                options=[
                    {
                    ...
                    {"label": lang("pregnancy", "Schwangerschaft"), "value": "preg"},
                ],
            ...
            input.select(
                lang("Evaluation of probation period", "Auswertung Probezeit"),
                options=[
                    {"label": lang("No", "Nein"), "value": False},
                    {"label": lang("Yes", "Ja"), "value": True},
                ],
                name="trial_relevance",
                required=True,
            ),
            input.select(
                lang("Evaluation of termination", "Auswertung Kündigung"),
                options=[
                    {"label": lang("No", "Nein"), "value": False},
                    {"label": lang("Yes", "Ja"), "value": True},
                ],
    ...
    with output.use_scope("scope_input_instructions", clear=True):
        output.put_markdown(
            lang(
                instructions["incapacity_amount"]["EN"],
                instructions["incapacity_amount"]["DE"],
            )
        )

    # User input: Amount of incapacities (block optional)
    if incapacity_type == "illacc":
        illacc_amount = input.select(
            lang(
                "Number of Seperate Incapacities (Illness or Accident)",
                "Anzahl unabhängiger Arbeitsunfähigkeiten (Krankheit oder Unfall)",
            ),
            options=[
                {
                    "label": lang(
                        "One single accident or illness",
                        "Einzelner Unfall oder Krankheit",
                    ),
                    "value": 1,
                },
                {
                    "label": lang(
                        "Two seperate accidents or illnesses",
                        "Zwei unabhängige Unfälle oder Krankheiten",
                    ),
                    "value": 2,
                },
                {
                    "label": lang(
                        "Three seperate accidents or illnesses",
                        "Drei unabhängige Unfälle oder Krankheiten",
                    ),
                    "value": 3,
                },
            ],
    ...
    with output.use_scope("scope_input_instructions", clear=True):
        output.put_markdown(
            lang(
                instructions["probation_period"]["EN"],
                instructions["probation_period"]["DE"],
            )
        )
            ...
            [
                ...
                input.select(
                    ...
                    [
                        ...
                        "2",
                        "3",
                        lang("No probation period", "Keine Probezeit"),
                    ],
                    name="trial_input",
                    type=input.TEXT,
                    required=True,
    ...
    with output.use_scope("scope_input_instructions", clear=True):
        output.put_markdown(
            lang(
                instructions["incapacity_illness_accident"]["EN"],
                instructions["incapacity_illness_accident"]["DE"],
            )
        )
    ...
    if illacc_amount == 3:
        ...
        # Sort dates into incap dict as list pairs on the third key
    ...
    with output.use_scope("scope_input_instructions", clear=True):
        output.put_markdown(
            lang(
                instructions["incapacity_milservice"]["EN"],
                instructions["incapacity_milservice"]["DE"],
            )
        )
    ...
    if incapacity_type == "milservice":
        milservice_data = input.input_group(
            ...
            [
                # Start of incapacity
                input.input(
                    lang("Start of service", "Dienstbeginn"),
                    name="milservice_sdt",
                    type=input.TEXT,
                    required=True,
                    pattern=DATE_PATTERN,
                ...
                # End of incapacity
                input.input(
                    lang("End of service", "Dienstende"),
                    name="milservice_edt",
                    type=input.TEXT,
                    required=True,
                    pattern=DATE_PATTERN,
    ...
    with output.use_scope("scope_input_instructions", clear=True):
        output.put_markdown(
            lang(
                instructions["incapacity_pregnancy"]["EN"],
                instructions["incapacity_pregnancy"]["DE"],
            )
        )
    ...
    if incapacity_type == "preg":
        preg_data = input.input_group(
            ...
            [
                ...
                input.input(
                    lang(
                        "Start date of pregnancy", "Datum des Schwangerschaftsbeginns"
                    ),
                    name="preg_sdt",
                    type=input.TEXT,
                    required=True,
                ...
                # End of incapacity
                input.input(
                    lang("Date of confinement", "Datum der Niederkunft"),
                    name="preg_edt",
                    type=input.TEXT,
                    required=True,
                    pattern=DATE_PATTERN,
    ...
    with output.use_scope("scope_input_instructions", clear=True):
        output.put_markdown(
            lang(instructions["termination"]["EN"], instructions["termination"]["DE"])
        )
    ...
    if termination_occurence == True:
        termination_data = input.input_group(
            ...
            [
                ...
                input.input(
                    lang(
                        "Date of termination notice receipt", "Datum Kündigungsempfang"
                    ),
                    name="termination_dt",
                    type=input.TEXT,
                    required=True,
                ...
                input.select(
                    lang("Termination date", "Kündigungstermin"),
                    [
                        lang(
                            "No mention of termination date",
                            "Keine Angaben zum Kündigungstermin",
                        ...
                        lang("Termination date anytime", "Kündungstermin jederzeit"),
                    ],
                    name="endpoint",
                    type=input.TEXT,
                    required=True,
    ...
    with output.use_scope("scope_input_instructions", clear=True):
        output.put_markdown(
            lang(
                instructions["probation_period_termination"]["EN"],
                instructions["probation_period_termination"]["DE"],
            )
        )
    ...
    if trial_relevance == True:
        ...
        trial_lst.insert(
            1, min(trial_lst[0].shift(months=+trial_dur), termination_dt)
        )  # BGer 4C.45/2004
        ...
        if trial_extension == True:
            for incap_sublst in incap_masterlst:
                ...
                for day in arrow.Arrow.range(
                    "days",
                    max(trial_lst[0], incap_sublst[0]),
                    min(trial_lst[1], incap_sublst[1]),
                ):
                    # Add date to list if it is a working day, not a holiday and not already in the list
                    if (
                        (day.weekday() in workdays_num)
                        and (day not in holidays)
                        and (day not in missed_workdays)
                    ):
                ...
                for day in arrow.Arrow.range(
                    "days", max(trial_lst[1], incap_sublst[1]).shift(days=+1), limit=365
                ):
                    if (
                        (day.weekday() in workdays_num)
                        and (day not in holidays)
                        and (len(missed_workdays) > len(repeated_workdays))
                    ):
                        repeated_workdays.append(day)

                # Set extension end date
                trial_lst[1] = min(
                    repeated_workdays[-1], termination_dt
                )  # cap at termination
    ...
    if incapacity_type == "illacc":
        ...
        for key, value in embargo_dct.items():
            ...
            for embargo_sublst in value:
                ...
                embargo_unclaimed_loop = max(
                    1, (embargo_cap_loop - embargo_claimed_loop)
                )

                # Set embargo start date
                embargo_sublst[0] = max(
                    reg_employment_lst[0], embargo_sublst[0]
                )  # starts on reg employment at the earliest
                ...
                if syears[1].is_between(embargo_sublst[0], embargo_sublst[1], "[)"):
                    ...
                    embargo_sublst[1] = min(
                        embargo_sublst[0].shift(days=(embargo_unclaimed_loop - 1)),
                        embargo_sublst[1],
                    )
                    # Count used days
                    embargo_claimed_loop = period_duration(
                        embargo_sublst[0], embargo_sublst[1]
                    )
                    # Skip syear cleanup
                    crossed_syear = 0
                ...
                if crossed_syear != 0:
                    ...
                    embargo_sublst[1] = min(
                        embargo_sublst[0].shift(days=(embargo_unclaimed_loop - 1)),
                        syears[crossed_syear].shift(days=-1),
                    )
                    ...
                    new_embargo_sublist.insert(
                        1,
                        min(
                            new_embargo_sublist[0].shift(
                                days=(embargo_unclaimed_loop - 1)
                            ),
                            save_date_embargo_split,
                        ),
                    )
                    ...
                    # Count used days
                    embargo_claimed_loop += period_duration(
                        new_embargo_sublist[0], new_embargo_sublist[1]
                    )
        ...
        if incapacity_type != False:
            ...
            if notice_overlap != 0:
                ...
                if not endpoint in [
                    ...
                    notice_ext_lst.insert(
                        1, push_endpoint(notice_comp_lst[1], endpoint)
                    )
    ...
    if incapacity_type == "illacc":
        ...
        for key, value in incap_dct.items():
            for incap_sublst in value:
                ...
                sickpay_sublst_1[0] = max(
                    employment_sdt.shift(months=+3), sickpay_sublst_1[0]
                )

                # Calculate seniority at the beginning of the incapacity
                # Source: https://stackoverflow.com/a/70038244/14819955
                sick_pay_syear_start_index = (
                    get_last_index(syears, lambda x: x < sickpay_sublst_1[0]) + 1
                )  # +1 to denote year 0 as first syear

                # Calculate seniority at the end of the incapacity
                sick_pay_syear_end_index = (
                    get_last_index(syears, lambda x: x < sickpay_sublst_1[1]) + 1
                )  # +1 to denote year 0 as first syear

                # Compare seniority at start and end, split if not the same
                # Group into dict according to start year
                if (
                    sick_pay_syear_start_index != sick_pay_syear_end_index
                ):  # not in the same year
                    # Hold original enddate
                    save_date_sick_pay_split = sickpay_sublst_1[1]
                    # Cap first period a day before syear
                    sickpay_sublst_1[1] = min(
                        sickpay_sublst_1[1],
                        syears[sick_pay_syear_start_index].shift(days=-1),
                    )
        ...
        for key, value in sickpay_dct.items():
            ...
            for sickpay_sublst in value:
                ...
                if syears[key] == syears[1]:
                    sick_pay_cap = period_duration(
                        sickpay_sublst[0], sickpay_sublst[0].shift(weeks=+3, days=-1)
                    )
                else:
                    # Add +1 to move query to the right index in sick pay matrix
                    sick_pay_cap = period_duration(
                        sickpay_sublst[0],
                        sickpay_sublst[0].shift(
                            **{unit: (pay_matrix[canton][key - 1])}, days=-1
                        ),
                    )
                ...
                sickpay_sublst[1] = min(
                    sickpay_sublst[0].shift(days=+sick_pay_unclaimed_loop - 1),
                    sickpay_sublst[1],
                    new_employment_edt,
                )

                # Count used sick days
                sick_pay_claimed_loop += period_duration(
                    sickpay_sublst[0], sickpay_sublst[1]
                )
    ...
    no_termination = lang(
        "[--> No termination evaluated]", "[--> Keine Kündigung evaluiert]"
    )
    ...
    if (termination_occurence == True) and termination_dt.is_between(
        trial_lst[0], trial_lst[-1], "[]"
    ):
    ...
    if termination_case == "no_case":
        ...
        reason = lang(
            "[--> No termination evaluated]", "[--> Keine Kündigung ausgewertet]"
        )
        new_employment_edt = lang(
            "[--> No termination evaluated]", "[--> Keine Kündigung ausgewertet]"
        )
    ...
    if termination_case == "trial_case":
        termination_validity = valid_termination
        reason = lang(
            "Termination during probation period.", "Kündigung während Probezeit."
        )

        # Set notice period according to user input
        if (trial_notice_input == "Not specified in contract") or (
            trial_notice_input == "Keine Angaben im Arbeitsvertrag"
        ):
    ...
    if termination_case == "embargo_case":
        ...
        reason = lang(
            "The termination was issued during an embargo period.",
            "Die Kündigung wurde während einer Sperrfrist ausgesprochen.",
        )
        notice_period_lst.clear()
        notice_overlap = 0
        reg_employment_lst[1] = reg_employment_lst[1].shift(
            years=+3
        )  # showing that employment continues
        notice_comp_lst.clear()
        notice_ext_lst.clear()
        new_employment_edt = lang(
            "[--> No valid termination]", "[--> Keine gültige Kündigung]"
        )
    ...
    if (termination_case == "standard_case") or (termination_case == "trial_case"):
        for sickpay_sublst in sickpay_masterlst:
            ...
            if new_employment_edt.is_between(
                sickpay_sublst[0], sickpay_sublst[1], "[]"
            ):
    ...
    with output.use_scope("scope_res_general"):
        output.put_markdown(
            lang("""## Key Results""", """## Wichtigste Resultate""")
        ).style("margin-top: 20px"),
        ...
        if trial_relevance != False:
            output.put_row(
                [
                    output.put_markdown(
                        lang(
                            """**Missed Workdays Probation Period:**""",
                            """**Verpasste Arbeitstage Probezeit:**""",
                        )
                    ),
        ...
        if termination_occurence != False:
            output.put_row(
                [
                    output.put_markdown(
                        lang(
                            """**Compensation Days Notice Period:**""",
                            """**Kompensationstage Kündigungsfrist:**""",
                        )
                    ),
    ...
    with output.use_scope("scope_res_trial"):
        output.put_markdown(
            lang("""## Detailed Results """, """## Detaillierte Ergebnisse""")
        ...
        if trial_relevance == True:
            ...
            tbl_trial_rows = [
                [
                    lang("Probation Period", "Probezeit"),
                    trial_lst[0].format("DD.MM.YYYY"),
                    trial_lst[1].format("DD.MM.YYYY"),
                    str(period_duration(trial_lst[0], trial_lst[1]))
                    + lang(" days", " Tage"),
                ]
            ]
            ...
            for key, value in incap_dct.items():
                for incap_sublst in value:
                    if incap_sublst != []:
                        ...
                        tbl_lst.append(
                            str(period_duration(incap_sublst[0], incap_sublst[1]))
                            + lang(" days", " Tage")
                        )
    ...
    with output.use_scope("scope_res_embargo_merged"):
        ...
        if incapacity_type != False:
            ...
            for embargo_sublst in embargo_masterlst:
                ...
                tbl_lst.append(
                    str(period_duration(embargo_sublst[0], embargo_sublst[1]))
                    + lang(" days", " Tage")
                )
            ...
            output.remove("scope_res_embargo_merged")

    # Scope for the summary of any sick pay periods
    with output.use_scope("scope_res_sp"):
        output.put_markdown(
            lang("""### Sick Pay Periods""", """### Perioden Lohnfortzahlung """)
        ).style("margin-top: 20px"),
        ...
        if incapacity_type != False:
            ...
            for sickpay_sublst in sickpay_masterlst:
                ...
                tbl_lst.append(
                    str(period_duration(sickpay_sublst[0], sickpay_sublst[1]))
                    + lang(" days", " Tage")
                )
        ...
        if (termination_occurence == True) and (termination_case != "embargo_case"):
            ...
            tbl_np_rows.append(
                [
                    ...
                    str(period_duration(notice_period_lst[0], notice_period_lst[1]))
                    + lang(" days", " Tage"),
                ]
            )
            ...
            try:
                tbl_np_rows.append(
                    [
                        lang(
                            "Notice Period Compensation", "Kompensation Kündigungsfrist"
                        ),
                        notice_comp_lst[0].format("DD.MM.YYYY"),
                        notice_comp_lst[1].format("DD.MM.YYYY"),
                        str(period_duration(notice_comp_lst[0], notice_comp_lst[1]))
                        + lang(" days", " Tage"),
                    ]
                )
            except IndexError:
            ...
            try:
                tbl_np_rows.append(
                    [
                        ...
                        str(period_duration(notice_ext_lst[0], notice_ext_lst[1]))
                        + lang(" days", " Tage"),
                    ]
                )
            except IndexError:
            ...
            output.remove("scope_res_notice")
    ...
    df_lst.append(
        pd.DataFrame(
            data=[
                [
                    lang(
                        "Compensation Missed Notice Period",
                        "Kompensation verpasste Kündigungsfrist",
                    ),
    ...
    fig = px.timeline(
        ...
        color_discrete_map={
            "[PH_T]": "#ffffff",
            "Sick Pay": "#f032e6",
            "Lohnfortzahlung": "#f032e6",
            "Probation Period": "#f58231",
            "Probezeit": "#f58231",
            "Regular Employment": "#3cb44b",
            "Reguläre Anstellung": "#3cb44b",
            "Notice Period": "#000075",
            "Kündigungsfrist": "#000075",
            "Compensation Missed Notice Period": "#4363d8",
            "Kompensation verpasste Kündigungsfrist": "#4363d8",
            "Sperrfrist": "#e6194B",
            "Embargo Period": "#e6194B",
            "Notice Period Extension": "#911eb4",
            "Verlängerung Kündigungsfrist": "#911eb4",
            "Incapacity": "#9A6324",
            "Arbeitsunfähigkeit": "#9A6324",
            "[PH_B]": "#ffffff",
        },
        width=1000,
        height=700,
        hover_name="task",
        hover_data={"task": False, "stack": False, "start": True, "end": True},
    )
    ...
    fig.update_layout(
        ...
        yaxis=dict(automargin=True, visible=False, autorange="reversed", showgrid=True),
        legend=dict(title="", orientation="h", font_size=16, x=0, y=1.1),
        shapes=[
            dict(
                x0=termination_dt,
                x1=termination_dt,
        ...
        annotations=[
            dict(
                ...
                xref="x",
                yref="paper",
                font=dict(size=16, color="#DB162F"),
                showarrow=False,
                xanchor="left",
                text=lang("Termination", "Kündigung"),
            ),
            dict(
                x=syears[1],
                y=0.05,
                xref="x",
                yref="paper",
                font=dict(size=16, color="#3B6728"),
                showarrow=False,
                xanchor="left",
                text="1Y",
            ),
    ...
    with output.use_scope("scope_visualization"):
        ...
        plotly_html = fig.to_html(
            include_plotlyjs="require", full_html=False, config=config
        )
        output.put_markdown(
            lang(
                instructions["visualization_disclaimer"]["EN"],
                instructions["visualization_disclaimer"]["DE"],
            )
        ).style("margin-top: 20px"),
        output.put_collapse(
            lang(
            ...
            [
                output.put_markdown(
                    lang(
                        instructions["visualization_instructions"]["EN"],
                        instructions["visualization_instructions"]["DE"],
                    )
                )
            ],
        ).style("margin-top: 20px"),
